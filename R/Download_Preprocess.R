...
#' 
#' For gene expression, this function downloads RNAseq data.
#' Pre-processing of gene expression data includes eliminating samples and genes with too many NAs, imputing NAs, and doing Batch correction.
#' 
...
#'  \item \code{data_CancerSite.rds}: list with both gene expression and methylation data. Methylation data is clustered and presented at the gene level. A matrix with the mapping from CpG sites to genes is included. A matrix
#' of sample patient data is included if avaliable.
#' }
#' @examples 
#' \dontrun{
  ...
  if (!grepl("Windows", Sys.info()['sysname'])) {
    ...
    if (downloadData) {
      ...
      if(fileType == "tar.gz" && untarUngzip) {                   
        ...
        # doesn't work because it calls system and system doesnt know about the virtual drive
        removed <- file.remove(paste0("x://", nameForDownloadedFile))
        # Anyway I change folder name to make it shorter
        changed <- file.rename(from = paste0("x://", gsub(".tar.gz", "", fileName)), 
                               to = paste0("x://", nameForFolder))
...
Preprocess_DNAmethylation <- function(CancerSite, MAEO, MissingValueThreshold = 0.2) {    
  ...
  if (!is.null(MAEO_27k)) {
    ...
    ProcessedData27k=Preprocess_CancerSite_Methylation27k(CancerSite,
                                                          MAEO_27k,
  ...
  if (!is.null(MAEO_450k)) {
    ...
    ProcessedData450k=Preprocess_CancerSite_Methylation450k(CancerSite,
                                                            MAEO_450k,
                                                            MissingValueThreshold)
  }
...
TCGA_Process_EstimateMissingValues <- function(MET_Data, MissingValueThreshold = 0.2) {
  ...
  if (is.na(sum(NrMissingsPerGene)) || sum(NrMissingsPerGene>MissingValueThreshold)>0) {
